...
# Removed: from diffusers import StableDiffusionPipeline, StableDiffusionImg2ImgPipeline 
from gtts import gTTS, lang as gtts_langs
import pandas as pd
import base64 # Required for API calls
import json   # Required for API calls

# --- API CONFIGURATION ---
# IMPORTANT: This URL points to your running Colab GPU instance. 
# It will expire when the Colab session ends, and must be updated if it changes.
COLAB_API_ENDPOINT = "https://rxxxxxxxxxxxxxxl-nxxxxxxxxxsferable.ngrok-free.dev" 
# -------------------------
...
# --------------------------
# DEFERRED STABLE DIFFUSION LOADING FUNCTIONS (Placeholder/API)
# --------------------------
@st.cache_resource
def load_visuals_models(pipe_type: str):
    # This function is now a placeholder and always returns True if the endpoint is set.
    # The actual heavy loading happens in the Colab backend.
    if COLAB_API_ENDPOINT == "YOUR_COLAB_API_ENDPOINT" or not COLAB_API_ENDPOINT:
        st.session_state.visuals_error = "API Endpoint missing. Update COLAB_API_ENDPOINT in app.py."
        return None
    return True # Placeholder for a successful "load"
...
def generate_healthy_leaf_img2img(base_pil_image: Image.Image, disease_label: str, strength=0.55, steps=25, guidance_scale=7.5):
    # This now sends a request to the Colab API
    if not load_visuals_models('img2img'):
        raise Exception(st.session_state.get('visuals_error', "API Endpoint not configured."))
    ...
    buffered = io.BytesIO()
    base_pil_image.convert("RGB").resize((512, 512)).save(buffered, format="JPEG")
    ...
    response = requests.post(f"{COLAB_API_ENDPOINT}/generate_image", headers=headers, data=json.dumps(payload), timeout=300) # 5 min timeout for GPU
    ...
    if result.get("status") != "success":
        raise Exception(f"GPU Execution Error: {result.get('detail', 'Unknown error')}")

    # Decode and return the resulting image
    output_image_data = base64.b64decode(result["image_base64"])
    return Image.open(io.BytesIO(output_image_data))


def generate_healthy_product_text2img(base_pil_image: Image.Image, disease_label: str, steps=30, guidance_scale=8.5):
    # This now sends a request to the Colab API
    if not load_visuals_models('text2img'):
        raise Exception(st.session_state.get('visuals_error', "API Endpoint not configured."))
    ...
    # Send base image (or dummy) as payload must match API model
    buffered = io.BytesIO()
    base_pil_image.convert("RGB").resize((1, 1)).save(buffered, format="JPEG") # Use a small dummy image
    img_base64 = base64.b64encode(buffered.getvalue()).decode('utf-8')

    payload = {
        "image_base64": img_base64, # Dummy image, ignored by text2img logic on backend
        "disease_label": disease_label,
        "request_type": "plant",
        "prompt": prompt
    }
...
def main():
    ...
    if 'visuals_running' not in st.session_state: st.session_state.visuals_running = False # NEW: Running flag
    ...
    if st.button(t("🔍 Analyze")):
        ...
        st.session_state.visuals_running = False # Reset running flag
        ...
        # --- KEY STATUS CHECK & ERROR HANDLING ---
    ...
    if st.session_state.analyzed:
        ...
        if st.session_state.prediction:
            ...
            if not st.session_state.visuals_generated and not st.session_state.visuals_running:
                if st.button(t("🎨 Generate AI Visuals (Slow)")):
                    if COLAB_API_ENDPOINT == "YOUR_COLAB_API_ENDPOINT" or not COLAB_API_ENDPOINT:
                        st.error(t("API Endpoint not set. Update COLAB_API_ENDPOINT in app.py."))
                    else:
                        st.session_state.visuals_running = True
                        st.rerun() # Rerun to enter the running state and show spinner immediately
            # --- END GENERATION BUTTON ---
            
            # --- RUNNING STATE ---
            if st.session_state.visuals_running:
                with st.spinner(t("Generating AI visuals... (Sending to GPU backend, please wait)")):
                    try:
                        # 1. Run the API calls (This is the long-running, blocking part)
                        leaf_img = generate_healthy_leaf_img2img(image, st.session_state.prediction)
                        plant_img = generate_healthy_product_text2img(image, st.session_state.prediction)
                        
                        # 2. Save results and switch to completed state
                        st.session_state.leaf_img = leaf_img
                        st.session_state.plant_img = plant_img
                        st.session_state.visuals_generated = True
                        st.session_state.visuals_running = False # Switch off running flag
            ...
            # --- WAITING STATE ---
            elif not st.session_state.visuals_generated and st.session_state.analyzed and not st.session_state.visuals_running:
                st.info(t("Click the 'Generate AI Visuals (Slow)' button above to create model images."))
